...
def calculate_triangle_leg():
    ...
    side_a = get_validated_float("Enter the length of side A: ")
    side_b = get_validated_float("Enter the length of side B: ")
    angle_c = get_validated_angle("Enter the measure of angle C in degrees: ")
...
def get_validated_angle(prompt):
    while True:
        try:
            value = float(input(prompt))
